...
| 6 | A control works when the browser tool clicks it and does nothing under a real mouse, so the check passes and the feature is still broken | `pointer-events: none` is inherited, and a dispatched `click` ignores hit testing entirely. The picture window is rendered inside a note row, whose content sits under a covering link with pointer events switched off, so the modal inherited it: every real click fell straight through the backdrop and the picture, and only Escape closed it. A dispatched click closed it every time, which is how the earlier fix was recorded as verified | The modal carries `pointer-events-auto` of its own. And any click a check depends on is driven as a REAL click at coordinates, never dispatched; a dispatched click proves the handler is wired, never that a person can reach it. When a click seems to do nothing, read the computed `pointer-events` of the target and of every ancestor before touching the handler | 2x | History 2026-09-08 and 2026-09-09 |
